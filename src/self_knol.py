...
Ensure that the structure is consistent and the information is detailed and accurate. Aim to provide at least 5 points for each subtopic, but you can include more if necessary. Stay strictly on the topic of {subject} and do not include information about unrelated subjects. Use only your own knowledge without referencing external sources."""
        ...
        if iteration == 1:
            ...
            query = f"""Improve the following knowledge entry about {subject}, taking into account the manager's review. 
...
Please provide an updated and improved version of the knol."""
        ...
        while True:
            ...
            while True:
                ...
                if rating >= 8.0:
                    ...
                    user_decision = input(color_user_input("Do you want to iterate again? (yes/no): ")).strip().lower()
                    if user_decision != 'yes':
                        print(warning("Knol creation process ended without meeting the required standard."))
                        break
                    iteration += 1
